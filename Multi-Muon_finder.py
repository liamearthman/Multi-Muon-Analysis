...
root_pattern=r"C:\Users\liame\Muons\ntuples\*.root" # all the root files in the data directory

root_files = glob.glob(root_pattern,recursive=True)
root_files.sort() # sort of sorts, but subrun numbers are not zero padded
if not root_files:
...
for file in root_files:
    with uproot.open(file) as f:
        ...
        if len(ts_high_branch) == 0:
            print(f"skipping empty file {file}")
            continue

        print(ts_high_branch)
        ...
        for ntracks in ntrack_tree:
            ...
            while i < ntracks:
                ...
                if size >= 6:
                    ...
                    double_list.append(
                    (run0, subrun0, art_event, size, time0, fileStart.strftime("%m")))
...
with open("muon_events2.txt", "w") as f:
    for run, subrun, event, size, time0, month in double_list:
        f.write(f"{run} {subrun} {event} {size} {time0} {month}\n")
